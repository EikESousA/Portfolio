...
describe('CreateSpotService', () => {
  ...
  it('should be able to reject a booking!', async () => {
    ...
    const request_reject_booking = await request(server)
      .post(`/aircnc/bookings/${create_booking._id}/rejections`)
      .expect(200);

    const reject_booking = request_reject_booking.body as IBooking;

    expect(reject_booking.approved).toEqual(false);
  ...
  it('should not be able to reject a booking because the booking does not exist!', async () => {
    await request(server)
      .post(`/aircnc/bookings/booking-not-exist/rejections`)
      .expect(400);
  }, 30000);
});
